...
<head>
    ...
    <link rel="icon" type="image/x-icon" href="../Logo/Logo/LogoBG.png">
...
<body class="bg-gray-100">
        ...
        <div class="container mx-auto px-4">
                ...
                <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
                    ...
                    <div>
                        ...
                        <select id="filterCapacity"
                            ...
                            <option>0 - 10,000 sq ft</option>
                            <option>10,001 - 50,000 sq ft</option>
                            <option>50,001 - 100,000 sq ft</option>
                            <option>100,001+ sq ft</option>
                        </select>
                    </div>
            ...
            <div class="bg-white shadow-md rounded-lg p-4 mb-6">
                <div class="flex justify-between items-center">
                    ...
                    <div class="flex space-x-2 no-print">
                        <button class="toggle-btn bg-blue-500 text-white px-4 py-2 rounded-lg text-sm hover:bg-blue-600"
                            onclick="toggleView('grid')">
                            <i class='bx bx-grid-alt mr-1'></i>Grid View
                        </button>
                        <button class="toggle-btn bg-gray-500 text-white px-4 py-2 rounded-lg text-sm hover:bg-gray-600"
            ...
            <!-- Warehouse Cards -->
            <div id="warehouseCards" class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 warehouse-grid">
                <?php foreach ($allwarehouses as $warehouse):
                    $available = (int)$warehouse['capacity_total'] - (int)$warehouse['capacity_used'];
                    $usagePercent = $warehouse['capacity_total'] > 0 ? ((int)$warehouse['capacity_used'] / (int)$warehouse['capacity_total']) * 100 : 0;
                ...
                ?>
                    <div data-city="<?= htmlspecialchars(strtolower($warehouse['location'])) ?>"
                        data-capacity-total="<?= (int)$warehouse['capacity_total'] ?>"
                        data-capacity-used="<?= (int)$warehouse['capacity_used'] ?>"
                        data-available="<?= max(0, $available) ?>" data-derived-status="<?= strtolower($derived) ?>"
                        onclick="window.location.href='manage_warehouse.php?id=<?= $warehouse['warehouse_id'] ?>'"
                        class="warehouse-card bg-white rounded-lg shadow-md overflow-hidden transition hover:shadow-xl cursor-pointer">
                        <div class="p-6">
                                ...
                                <div class="flex items-center">
                                    ...
                                    <div>
                                        <h3 class="text-lg font-semibold text-gray-900">
                                            <?= htmlspecialchars($warehouse['name']) ?></h3>
                                        <p class="text-sm text-gray-500">ID: WH-<?= $warehouse['warehouse_id'] ?></p>
                                    </div>
                                </div>
                                <span class="px-3 py-1 rounded-full text-xs font-medium <?= $statusColors[$derived] ?>">
                            ...
                            <div class="grid grid-cols-2 gap-4 mb-6">
                                <div class="text-sm text-gray-600">
                                    <div class="flex items-center mb-2">
                                        ...
                                        <span><?= htmlspecialchars($warehouse['location']) ?></span>
                                    </div>
                                    <div class="flex items-center">
                                        <i class='bx bx-category mr-2 text-gray-400'></i>
                                        <span><?= htmlspecialchars($warehouse['type']) ?></span>
                                    </div>
                                </div>
                                <div class="text-sm text-gray-600">
                                    <div class="flex items-center mb-2">
                                        <i class='bx bx-cube mr-2 text-gray-400'></i>
                                        <span>Total: <?= number_format($warehouse['capacity_total']) ?> sq ft</span>
                                    </div>
                                    <div class="flex items-center">
                                        <i class='bx bx-layer mr-2 text-gray-400'></i>
                                        <span class="<?= $available > 0 ? 'text-green-600' : 'text-red-600' ?>">
                                            Available: <?= number_format(max(0, $available)) ?> sq ft
                                        </span>
                                    </div>
                                </div>
                            </div>
                            ...
                            <div>
                                <div class="flex justify-between items-center mb-2 text-sm">
                                    <span class="font-medium text-gray-700">Capacity Usage</span>
                                    <span class="font-semibold text-gray-900"><?= number_format($usagePercent, 1) ?>%</span>
                                </div>
                                <div class="bg-gray-200 rounded-full h-2.5 mb-1 capacity-bar">
                                    <div class="h-2.5 rounded-full capacity-fill <?= $usagePercent >= 90 ? 'bg-red-500' : ($usagePercent >= 70 ? 'bg-yellow-500' : 'bg-green-500') ?>"
                                        style="width: <?= min(100, $usagePercent) ?>%"></div>
                                </div>
                                <p class="text-xs text-gray-500 text-right">
                                    <?= number_format($warehouse['capacity_used']) ?> used
                                </p>
                            </div>
                        </div>
                    </div>
    ...
    <script>
        ...
        function isInCapacityRange(card, capacity) {
            ...
            if (capacity === '0 - 10,000 sq ft') return total >= 0 && total <= 10000;
            if (capacity === '10,001 - 50,000 sq ft') return total >= 10001 && total <= 50000;
            if (capacity === '50,001 - 100,000 sq ft') return total >= 50001 && total <= 100000;
            if (capacity === '100,001+ sq ft') return total >= 100001;
            return true; // 'All'
        }
        ...
        function toggleView(viewType) {
            ...
            toggleBtns.forEach(btn => {
                ...
                btn.classList.add(viewType === 'grid' && btn.textContent.includes('Grid') ? 'bg-blue-500' :
                    'bg-gray-500');
            });

            // Update grid class
            if (viewType === 'list') {
                warehouseGrid.className = 'grid grid-cols-1 gap-6 warehouse-grid';
            } else {
                warehouseGrid.className = 'grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 warehouse-grid';
            }
        }
