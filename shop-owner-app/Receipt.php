...
<body class="bg-gray-100">
    ...
    <section class="home-section p-0">
        <div class="flex justify-between items-center p-4">
            ...
            <button onclick="showInventoryStatus()" class="bg-green-500 text-white px-3 py-1 rounded text-sm hover:bg-green-600">
        ...
        <div class="container mx-auto my-3">
            ...
            <?php if (!empty($_SESSION['cart'])): ?>
                ...
                ?>
                <div class="bg-white shadow-md rounded-lg mb-3 flex cart-item" data-product="<?php echo $item['product_id']; ?>">
                    <div class="w-1/6 p-2">
                    ...
                    <div class="w-4/6 flex flex-col justify-center p-4">
                        ...
                        <p class="text-gray-600">Price: ৳<?php echo number_format($item['price'], 2); ?> per <?php echo htmlspecialchars($item['unit']); ?></p>
                    ...
                    <div class="w-1/6 flex flex-col items-center justify-center">
                        <div class="flex items-center space-x-2 mb-2">
                            <form method="post" style="display: inline;">
                                ...
                                <button type="submit" class="decrease-btn bg-red-200 hover:bg-red-300 px-3 py-1 rounded">-</button>
                            </form>
                            <div class="counter text-lg font-semibold"><?php echo $item['quantity']; ?></div>
                            <form method="post" style="display: inline;">
                                <input type="hidden" name="product_id" value="<?php echo $item['product_id']; ?>">
                                <input type="hidden" name="cart_action" value="add">
                                <button type="submit" class="increase-btn bg-green-200 hover:bg-green-300 px-3 py-1 rounded">+</button>
                            </form>
                        </div>
                    </div>
                </div>
                ...
                <div class="bg-white shadow-md rounded-lg mb-3 p-4">
                    <div class="flex justify-between items-center">
                        ...
                        <div class="space-x-2">
                            ...
                            <button class="bg-green-500 text-white px-4 py-2 rounded hover:bg-green-600" onclick="generateReceipt()">Generate Receipt & Process Sale</button>
                        </div>
                    </div>
                </div>
            <?php else: ?>
    ...
    <div id="manual-modal" class="fixed inset-0 bg-black bg-opacity-75 flex items-center justify-center z-50 hidden modal">
        <div class="bg-white p-6 rounded-lg max-w-4xl w-full mx-4 max-h-96 overflow-hidden">
            ...
            <div class="flex justify-end space-x-2">
                <button onclick="closeManualModal()" class="bg-gray-500 text-white px-4 py-2 rounded hover:bg-gray-600">Cancel</button>
            </div>
        </div>
    </div>

    <!-- Webcam Scanning Modal -->
    <div id="webcam-modal" class="fixed inset-0 bg-black bg-opacity-75 flex items-center justify-center z-50 hidden modal">
        <div class="bg-white p-4 rounded-lg max-w-md w-full mx-4">
            <h3 class="text-lg font-bold mb-2">Scanning for Products...</h3>
            <video id="webcam-video" autoplay class="w-full max-h-64 object-cover rounded mb-2" style="background: black;"></video>
    ...
    <!-- Receipt Modal -->
    <div id="receipt-modal" class="fixed inset-0 bg-black bg-opacity-75 flex items-center justify-center z-50 hidden modal">
        <div class="bg-white p-6 rounded-lg max-w-md w-full mx-4">
            ...
            <div class="flex justify-end space-x-2 mt-4 no-print">
                ...
                <button onclick="closeReceiptModal()" class="bg-gray-500 text-white px-4 py-2 rounded hover:bg-gray-600">Close</button>
            </div>
        </div>
    </div>

    <!-- Inventory Status Modal -->
    <div id="inventory-modal" class="fixed inset-0 bg-black bg-opacity-75 flex items-center justify-center z-50 hidden modal">
        <div class="bg-white p-6 rounded-lg max-w-md w-full mx-4">
            ...
            <div class="flex justify-end space-x-2 mt-4">
                <button onclick="closeInventoryModal()" class="bg-gray-500 text-white px-4 py-2 rounded hover:bg-gray-600">Close</button>
            </div>
        </div>
    </div>
    ...
    function searchProducts(query) {
        ...
        .then(products => {
            ...
            if (products.length === 0) {
                resultsDiv.innerHTML = '<p class="text-gray-500 text-center">No products found in store inventory</p>';
                return;
            }
            ...
            products.forEach(product => {
                ...
                productDiv.className = 'flex items-center p-3 border-b hover:bg-gray-50 cursor-pointer search-result';
                ...
                if (video.readyState === video.HAVE_ENOUGH_DATA && !detected) {
                    ...
                    try {
                        ...
                        if (predictions.predictions && predictions.predictions.length > 0 && 
                            ...
                            const detectedClass = predictions.predictions[0].class.toLowerCase();
                            const productId = productMappings[detectedClass];
                            
                            if (productId) {
                                status.textContent = `Detected ${predictions.predictions[0].class}! Adding to cart...`;
                                detected = true;
                                ...
                                status.textContent = `Detected ${predictions.predictions[0].class} but no matching product found.`;
                            }
                        } else {
                            status.textContent = 'No products detected. Try adjusting the angle or lighting.';
                        }
                    } catch (apiError) {
